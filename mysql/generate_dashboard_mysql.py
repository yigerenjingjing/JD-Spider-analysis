# -*- coding: utf-8 -*-
"""从 MySQL 数据库读取京东爬虫数据并生成可视化 HTML 看板。

使用 jieba + TF-IDF 提取关键词，包含统计分析（Pearson 相关、t 检验）和词云。

用法:
    python generate_dashboard_mysql.py
    python generate_dashboard_mysql.py --output my_dashboard.html
    python generate_dashboard_mysql.py --host 192.168.1.100 --password xxx
"""
...
from mysql.store_to_mysql import JDSpiderDB, load_all_data, DB_CONFIG
...
def keyword_analysis(comments_data):
    ...
    for item in comments_data:
        ...
        tfidf_kw = jieba.analyse.extract_tags(all_text, topK=15, withWeight=True)
...
def global_keywords_tfidf(comments_data):
    ...
    tfidf_kw = jieba.analyse.extract_tags(all_text, topK=30, withWeight=True)
    filtered = [(w, round(s, 1)) for w, s in tfidf_kw if w not in STOPWORDS and len(w) >= 2]
...
def wordcloud_data(comments_data):
    all_text = " ".join(c for item in comments_data for c in item["comments"])
    tfidf_kw = jieba.analyse.extract_tags(all_text, topK=100, withWeight=True)
...
def pearson_price_sales(products):
    prices, sales = [], []
    for p in products:
        try:
...
def ttest_review_length(comments_data):
    pos_lengths, neg_lengths = [], []
    for item in comments_data:
        for i, c in enumerate(item["comments"]):
            s = item.get("sentiments", [])
            if i < len(s):
                if s[i] == "positive":
...
def sentiment_from_db(comments_data):
    results = []
    for item in comments_data:
        sentiments = {"positive": 0, "neutral": 0, "negative": 0}
        for s in item.get("sentiments", []):
            if s in sentiments:
                sentiments[s] += 1
        results.append({
            "product": item["product_name"],
...
def generate_html(products, comments_data, output_path):
    sentiments = sentiment_from_db(comments_data)
    ...
    pearson_r, pearson_p, prices_list, sales_list = pearson_price_sales(products)
    t_stat, t_pval, pos_lens, neg_lens = ttest_review_length(comments_data)
...
<title>京东宠物玩具数据分析看板 (MySQL)</title>
...
<div class="header">
  <h1>🐾 京东宠物玩具数据分析看板</h1>
  <p>基于用户评论的多维度数据可视化分析 · jieba + TF-IDF · 数据来源：MySQL 数据库</p>
</div>
...
<div class="container">
  ...
  <div class="stats-row">
    ...
    <div class="stat-card">
      ...
      <div><div class="stat-value">{round(sum(p.get('good_rate',0) for p in products)/max(len(products),1), 1)}%</div><div class="stat-label">平均好评率</div></div>
    </div>
    <div class="stat-card">
  ...
  <div class="table-card">
    ...
    <div style="overflow-x:auto;">
      <table>
        ...
        <tbody>
          {''.join(f'''<tr>
            <td>{i+1}</td><td>{p.get('product_name','')[:30]}</td><td>{p.get('price','')}</td><td>{p.get('store_name','')[:18]}</td>
            <td>{p.get('sales_text','')}</td><td>{p.get('good_rate',0)}%</td>
            <td>{s['total']}</td>
...
<div class="footer">京东宠物玩具数据可视化看板 · jieba + TF-IDF · ECharts · 数据源: MySQL · 自动生成于 {datetime.now().strftime('%Y-%m-%d %H:%M')}</div>

<script>
const PRODUCTS = {json.dumps(products, ensure_ascii=False, default=str)};
...
const prodNames = PRODUCTS.map((p,i) => '#'+(i+1)+' '+(p.product_name||'').slice(0,10));
const priceData = PRODUCTS.map(p => parseFloat((p.price||'').replace('¥',''))||0);

(function(){{
  const c=echarts.init(document.getElementById('chart-price'));
  c.setOption({{tooltip:{{trigger:'axis',axisPointer:{{type:'shadow'}}}},grid:{{left:'3%',right:'10%',bottom:'10%',top:'10%',containLabel:true}},xAxis:{{type:'category',data:prodNames,axisLabel:{{rotate:20,fontSize:11}}}},yAxis:{{type:'value',name:'价格 (¥)'}},series:[{{type:'bar',data:priceData,itemStyle:{{color:new echarts.graphic.LinearGradient(0,0,0,1,[{{offset:0,color:'#e74c3c'}},{{offset:1,color:'#f1948a'}}])}},label:{{show:true,position:'top',formatter:'¥{{c}}',fontSize:12}},barMaxWidth:50}}]}});
  window.addEventListener('resize',()=>c.resize());
}})();

(function(){{
  const c=echarts.init(document.getElementById('chart-sales-rate'));
  c.setOption({{tooltip:{{trigger:'axis',axisPointer:{{type:'cross'}}}},legend:{{data:['销量','好评率'],bottom:0}},grid:{{left:'3%',right:'10%',bottom:'15%',top:'10%',containLabel:true}},xAxis:{{type:'category',data:prodNames,axisLabel:{{rotate:20,fontSize:11}}}},yAxis:[{{type:'value',name:'销量'}},{{type:'value',name:'好评率 (%)',min:90,axisLabel:{{formatter:'{{value}}%'}}}}],series:[{{name:'销量',type:'bar',data:PRODUCTS.map(p=>p.sales_num||0),itemStyle:{{color:'#1890ff'}},barMaxWidth:40,label:{{show:true,position:'top',formatter:p=>{{const v=p.value;return v>=10000?(v/10000).toFixed(0)+'万':v}},fontSize:11}}}},{{name:'好评率',type:'line',yAxisIndex:1,data:PRODUCTS.map(p=>p.good_rate||0),lineStyle:{{color:'#52c41a',width:3}},itemStyle:{{color:'#52c41a'}},symbol:'circle',symbolSize:10,label:{{show:true,formatter:'{{c}}%',fontSize:12,color:'#52c41a'}}}}]}});
  window.addEventListener('resize',()=>c.resize());
}})();

(function(){{
  const c=echarts.init(document.getElementById('chart-sentiment-bar'));
  c.setOption({{tooltip:{{trigger:'axis',axisPointer:{{type:'shadow'}}}},legend:{{data:['正面','中性','负面'],bottom:0}},grid:{{left:'3%',right:'4%',bottom:'15%',top:'5%',containLabel:true}},xAxis:{{type:'category',data:prodNames,axisLabel:{{rotate:20,fontSize:11}}}},yAxis:{{type:'value',name:'评论数'}},series:[{{name:'正面',type:'bar',stack:'total',data:SENTIMENTS.map(s=>s.sentiments.positive),itemStyle:{{color:'#52c41a'}},barMaxWidth:50}},{{name:'中性',type:'bar',stack:'total',data:SENTIMENTS.map(s=>s.sentiments.neutral),itemStyle:{{color:'#faad14'}},barMaxWidth:50}},{{name:'负面',type:'bar',stack:'total',data:SENTIMENTS.map(s=>s.sentiments.negative),itemStyle:{{color:'#e74c3c'}},barMaxWidth:50}}]}});
  window.addEventListener('resize',()=>c.resize());
}})();

(function(){{
  const tP=SENTIMENTS.reduce((a,s)=>a+s.sentiments.positive,0),tN=SENTIMENTS.reduce((a,s)=>a+s.sentiments.neutral,0),tNg=SENTIMENTS.reduce((a,s)=>a+s.sentiments.negative,0);
  const c=echarts.init(document.getElementById('chart-sentiment-pie'));
  c.setOption({{tooltip:{{trigger:'item',formatter:'{{b}}: {{c}} 条 ({{d}}%)'}},legend:{{orient:'vertical',left:'left',top:'center'}},series:[{{type:'pie',radius:['45%','72%'],center:['55%','50%'],emphasis:{{label:{{fontSize:18,fontWeight:'bold'}}}},label:{{formatter:'{{b}}\\n{{c}} 条 ({{d}}%)'}},data:[{{value:tP,name:'正面评价',itemStyle:{{color:'#52c41a'}}}},{{value:tN,name:'中性评价',itemStyle:{{color:'#faad14'}}}},{{value:tNg,name:'负面评价',itemStyle:{{color:'#e74c3c'}}}}]}}]}});
  window.addEventListener('resize',()=>c.resize());
}})();

(function(){{
  const c=echarts.init(document.getElementById('chart-wordcloud'));
  c.setOption({{tooltip:{{formatter:'{{b}}: TF-IDF={{c}}'}},series:[{{type:'wordCloud',shape:'circle',left:'center',top:'center',width:'90%',height:'90%',sizeRange:[14,60],rotationRange:[-45,45],rotationStep:45,gridSize:8,drawOutOfBound:false,layoutAnimation:true,textStyle:{{fontFamily:'"Microsoft YaHei", sans-serif',fontWeight:'normal',color:function(){{const colors=['#e74c3c','#1890ff','#52c41a','#fa8c16','#722ed1','#eb2f96','#13c2c2','#f5222d','#2f54eb','#faad14'];return colors[Math.floor(Math.random()*colors.length)]}}}},emphasis:{{textStyle:{{fontSize:28,fontWeight:'bold'}}}},data:WORDCLOUD_DATA}}]}});
  window.addEventListener('resize',()=>c.resize());
}})();

(function(){{
  const c=echarts.init(document.getElementById('chart-keywords'));
  const e=Object.entries(GLOBAL_KW);
  c.setOption({{tooltip:{{trigger:'axis',axisPointer:{{type:'shadow'}}}},grid:{{left:'2%',right:'15%',bottom:'3%',top:'3%',containLabel:true}},xAxis:{{type:'value',name:'TF-IDF 权重'}},yAxis:{{type:'category',data:e.map(e=>e[0]).reverse(),axisLabel:{{fontSize:12}},inverse:true}},series:[{{type:'bar',data:e.map(e=>e[1]).reverse(),itemStyle:{{color:new echarts.graphic.LinearGradient(0,0,1,0,[{{offset:0,color:'#722ed1'}},{{offset:1,color:'#b37feb'}}])}},label:{{show:true,position:'right',fontSize:11}},barMaxWidth:24}}]}});
  window.addEventListener('resize',()=>c.resize());
}})();

(function(){{
  const c=echarts.init(document.getElementById('chart-length'));
  const bd=LENGTH_STATS.map((s,i)=>{{const so=[...s.lengths].sort((a,b)=>a-b);return[s.min,so[Math.floor(so.length*.25)],so[Math.floor(so.length*.5)],so[Math.floor(so.length*.75)],s.max]}});
  c.setOption({{tooltip:{{trigger:'axis'}},grid:{{left:'8%',right:'5%',bottom:'10%',top:'10%'}},xAxis:{{type:'category',data:prodNames,axisLabel:{{rotate:20,fontSize:11}}}},yAxis:{{type:'value',name:'字数'}},series:[{{type:'boxplot',data:bd,itemStyle:{{color:'#722ed1',borderColor:'#531dab'}}}},{{type:'scatter',data:LENGTH_STATS.map((s,i)=>[i,s.avg]),symbolSize:12,itemStyle:{{color:'#e74c3c'}},label:{{show:true,formatter:p=>'均'+p.value,position:'top',fontSize:11}}}}]}});
  window.addEventListener('resize',()=>c.resize());
}})();

(function(){{
  const c=echarts.init(document.getElementById('chart-scatter'));
  c.setOption({{title:{{text:PEARSON_R!==null?'r = '+PEARSON_R:'',left:'center',top:5,textStyle:{{fontSize:14,color:'#722ed1'}}}},tooltip:{{trigger:'item',formatter:'价格: ¥{{c[0]}}<br/>销量: {{c[1]}}'}},grid:{{left:'12%',right:'5%',bottom:'10%',top:'15%'}},xAxis:{{type:'value',name:'价格 (¥)',nameLocation:'center',nameGap:30}},yAxis:{{type:'value',name:'销量',nameLocation:'center',nameGap:40,axisLabel:{{formatter:v=>v>=10000?(v/10000).toFixed(0)+'万':v}}}},series:[{{type:'scatter',data:SCATTER_DATA.map(d=>[d.price,d.sales]),symbolSize:16,itemStyle:{{color:'#1890ff'}}}}]}});
  window.addEventListener('resize',()=>c.resize());
}})();

KEYWORDS.forEach((item,i)=>{{
  const c=echarts.init(document.getElementById('chart-kw-'+i));
  const e=Object.entries(item.keywords);
  c.setOption({{tooltip:{{trigger:'axis',axisPointer:{{type:'shadow'}}}},grid:{{left:'2%',right:'15%',bottom:'3%',top:'3%',containLabel:true}},xAxis:{{type:'value',name:'TF-IDF'}},yAxis:{{type:'category',data:e.map(e=>e[0]).reverse(),axisLabel:{{fontSize:11}},inverse:true}},series:[{{type:'bar',data:e.map(e=>e[1]).reverse(),itemStyle:{{color:new echarts.graphic.LinearGradient(0,0,1,0,[{{offset:0,color:'#1890ff'}},{{offset:1,color:'#69c0ff'}}])}},label:{{show:true,position:'right',fontSize:10}},barMaxWidth:20}}]}});
  window.addEventListener('resize',()=>c.resize());
}});
</script>
</body>
...
def main():
    ...
    parser = argparse.ArgumentParser(description="从 MySQL 生成数据可视化看板")
    ...
    output_path = args.output or os.path.join(os.path.dirname(__file__), "dashboard.html")

    print("[系统] 从 MySQL 读取数据...", flush=True)
    products, comments_data = load_all_data(config)

    if not products:
        print("[错误] 数据库中没有数据，请先运行爬虫。", flush=True)
        sys.exit(1)

    print(f"[系统] 读取到 {len(products)} 个商品, {sum(len(c['comments']) for c in comments_data)} 条评论", flush=True)
    generate_html(products, comments_data, output_path)
